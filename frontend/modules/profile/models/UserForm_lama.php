...
class UserForm extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            //['username', 'unique', 'targetClass' => '\common\models\User', 'message' => 'username all ready exis.'],
            ['username', 'validateUniqueUsername'],

			['email','required'],
            ['email', 'filter', 'filter' => 'trim'],
			['email', 'email'],
            ['email', 'unique', 'targetClass' => '\common\models\User', 'message' => 'email all ready exis.'],
        ];
    }
    ...
    public function validateUniqueUsername($attribute, $params)
    {
        if ($this->hasErrors()) {          
            ...
            if ($this->username === $user->username) {
                $this->addError($attribute, 'username all ready exis.');
            }
        }
    }
